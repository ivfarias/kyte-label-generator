...
async function scrapeSearchEngine(query, limit = 10) {
    ...
    try {
        ...
        const searchResults = await page.evaluate((limit) => {
            return Array.from(document.querySelectorAll('#b_results > li.b_algo')).slice(0, limit).map(item => ({
                url: item.querySelector('a')?.href || ''
            }));
        }, limit);

        await browser.close();
        console.log(`Fetched URLs for query "${query}":`, searchResults);
...
async function competitorAnalysis(progressBar) {
    ...
    for (const keyword of answers.keywords) {
        ...
        const { searchResults } = await scrapeSearchEngine(keyword, 10);
        console.log(`Parsed URLs for keyword "${keyword}":`, searchResults);
    ...
    console.log("All parsed URLs:", allUrls);
    console.log("URL counts:", urlCounts);
...
async function main() {
    ...
    if (moduleSelection.module === 'SEO Analysis') {
        ...
        progressBar.update(15, {
            status: 'Scraping Search Engine...'
        });
        const data = await scrapeSearchEngine(answers.query, progressBar);
        progressBar.update(40, {
            status: 'Search Engine Scraping Complete'
        });
        ...
        let openAIPrompt =
            `You'll use ${paaText} as well as ${piQuestionsText} to determine if the competitors are covering that information Search Engine deems relevant for the subject. 
        ...
        if (secondAnalysisPrompt.conductSecondAnalysis) {
            ...
            let aiAnalysisText = structuredData.Competitors.map(comp => comp
                .AIAanalysis).join('\n\n');
            let secondAIPrompt =
                `Based on ${paaText} and ${piQuestionsText} as well as the following AI analysis conducted for each Competitor:\n\n${aiAnalysisText}\n\nOutline the content structure for an optimized landing page that'll add more value to users and rank higher on Search Engine.`;
